...
# База данных взята с https://www.kaggle.com/tanuprabhu/population-by-country-2020
...
def is_local_run() -> bool:
    host_name = platform.platform()
    return 'Tikinokys' in host_name


global dir_path
dir_path = '' if is_local_run() else '/tmp/'
print(dir_path)
...
def read_system_credentials():
    ...
    if bool(prefs) is False:
        print('Невозможно продолжить. Системные учетные данные не настроены.')
        return
...
def write_timeseries_data(json_data: str):
    # сохранение файла
    with open(get_timeseries_data_path(), 'w') as outfile:  # создание или перезапись
...
def read_pref_date() -> int:
    if os.path.exists(get_prefs_path()) is False:
        return 0
    config = read_prefs()
    datetime_val = config.get('datetime')
