...
def generate_world_mortality_rate_10() -> Optional[Tuple[Any, Any]]:
    ...
    for country_title, dates in data_items:
        if country_title == 'MS Zaandam':
            continue
        latest_date = dates[-1]
        confirmed = latest_date[StatType.CONFIRMED.to_data_name()]
        deaths = latest_date[StatType.DEATHS.to_data_name()]
    ...
    for k, v in most_areas:
        ...
        if country_data is not None:
            ...
            for date in country_data:
                confirmed = date[StatType.CONFIRMED.to_data_name()]
                deaths = date[StatType.DEATHS.to_data_name()]
                if confirmed == 0: continue
                mortality_rate: float = deaths / confirmed

                cell_value = mortality_rate * 100  # конвертация в проценты
                date_str: str = date['date']
                dt: datetime = parse_date(date_str)
                x.append(dt)
                y.append(cell_value)

            ax.plot(x, y, label=country_title)  # для каждой страны
    ...
    for k, v in most_areas:
        ...
        for date in dates:
            ...
            if active:
                if cell_value < 1:
                    continue
                # поскольку api означает только общие значения
        ...
        ax.plot(x, y, linewidth=line_width, label=country_title)  # для каждой страны
...
def generate_bar_world_stat_10(stat_type: StatType) -> Optional[Tuple[Any, Any]]:
    ...
    for country_title, dates in data.items():
        total = dates[-1][stat_type.to_data_name()]
    ...
    for i, count in enumerate(y):
        ax.text(count, i, " " + f'{count:,}', color='black', va='center')  # печать больших чисел с запятой
...
def generate_country_active_plot(country: Country, stat_type: StatType) -> Optional[Tuple[Any, Any]]:
    ...
    ax.plot(x, y, marker='', linewidth=1.5, label=stat_type.to_title().title())  # для каждой страны
    if len(y_recovered) > 0:
        ax.plot(x, y_recovered, marker='', linewidth=1.5,
                label=StatType.RECOVERED.to_title().title())  # для каждой страны
        plt.legend(loc="upper left")
...
def generate_country_active_plot_per_million(country: Country, stat_type: StatType) -> Optional[
    ...
    if people == 0:  # нет такой страны
        return None
    person_per_million = people // 1_000_000
    ...
    ax.plot(x, y, marker='', color='#EF7028', linewidth=2.5, label=country_name)  # для каждой страны
...
def get_plot_country_per_million(country_data: list, stat_type: StatType, person_per_million: int, fun, *args,
    ...
    for idx, item in enumerate(country_data):
        ...
        cases_per_million = actual_diff // person_per_million
        if cases_per_million < 3:  # показывает только те, которые достигли 3 случаев на миллион
            continue
...
def generate_world_stat_10_per_million(stat_type: StatType) -> Optional[Tuple[Any, Any]]:
    ...
    for k, v in most_areas:
        ...
        # передать пользовательскую функцию для вычисления значения Y
        op_calc_val = lambda y: return_per_million_val(y, person_per_million)
        x, y = get_axis_avg_week_plot(dates, stat_type, op_calc_val, False)

        ax.plot(x, y, label=country_name)  # для каждой страны
...
def get_axis_avg_week_plot(country_data: list, stat_type: StatType, op_calc_val=lambda y: return_input_arg(y),
    ...
    counter = 0
    skip_first_empty = True  # пропустить первые пустые данные
    for idx, item in enumerate(country_data):
        counter = counter + 1
        cell_value = virus_utils.num(item[stat_type.to_data_name()])
        ...
        if index >= window_size:
            summa = summa - y[index - window_size]
            avg_val = summa // window_size
        else:
            avg_val = summa // len(y)

        output = op_calc_val(avg_val)
        if output < 3:  # пропускать почти пустые значения
            continue
...
def generate_toll_plot_avg(stat_type: StatType) -> Optional[Tuple[Any, Any]]:
    ...
    for k, v in most_areas:
        ...
        ax.plot(x, avg, label=country_name)  # для каждой страны
...
def draw_text_bar_vert(bar: Any):
    # Добавляет счетчики над двумя гистограммами
    for rect in bar:
...
def generate_country_total_plot(country: Country, stat_type: StatType) -> Optional[
    ...
    ax.plot(x, y, marker='', linewidth=2, label=stat_type.to_title().title())  # для каждой страны
    if len(y_recovered) > 0:
        ax.plot(x, y_recovered, marker='', linewidth=2, label=StatType.RECOVERED.to_title().title())  # для каждой страны
        plt.legend(loc="upper left")
